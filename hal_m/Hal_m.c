...
void producitabella(int pipe_fd[2]);
void generatesto(int pipe_fd[2]);
void crea_array(int pipe_fd[2]);
...
int main(int args, char *argv[])
{
    ...
    else
    {
        ...
        if (pipe(pipe_fd1) == -1)
        {
            perror("Errore nella creazione della pipe 1");
            exit(1);
        }

        if (pipe(pipe_fd2) == -1)
        {
            perror("Errore nella creazione della pipe 2");
            exit(1);
        }

        pid_t pid1, pid2, pid3;

        // Creazione del primo processo: producitabella
        if ((pid1 = fork()) == -1)
        {
            perror("Errore nella fork di produciTabellaOccorrenze");
            exit(1);
        }

        if (pid1 == 0)
        {
            produciTabellaOccorrenze(pipe_fd1);
        }

        // Creazione del secondo processo: crea_array
        if ((pid2 = fork()) == -1)
        {
            perror("Errore nella fork di crea_array");
            exit(1);
        }
        ...
        // Creazione del terzo processo: generatesto
        if ((pid3 = fork()) == -1)
        {
            perror("Errore nella fork di generatesto");
            exit(1);
        }

        if (pid3 == 0)
        {
            generatesto(pipe_fd2);
        }

        // Chiudi i lati delle pipe nel processo padre
        close(pipe_fd1[0]);
        close(pipe_fd1[1]);
        close(pipe_fd2[0]);
        close(pipe_fd2[1]);
...
void crea_array(int pipe_fd[2])
{
    ...
    read(pipe_fd[0], buffer, 2); // Attendi l'OK
    buffer[2] = '\0';
    if (strcmp(buffer, "OK") == 0)
    {
        // Esegui operazioni...
        printf("Crea array eseguito\n");
    }
...
/// @brief funzione per la lettura del file in input e per la creazione degli array con le occorrenze.

void produciTabellaOccorrenze(int pipe_fd[2])
{

    close(pipe_fd[0]); // Chiudo la pipe di lettura
    ...
    if (fin != NULL)
        fclose(fin);
    // abilitare la stampa se si vuole visualizzare il file normalizzato
    // stampaArrayCaratteri(fileNormalizzato, lenFileNormalizzato, "File Normalizzato");
    popolaArrayParoleDistinte(fileNormalizzato);
    // abilitare la stampa de si vuole visualizzare l'array delle parole distinte
    ...
    // Esegui operazioni...
    printf("Tabella Occorrenza prodotta\n");
    write(pipe_fd[1], "OK", 2); // Comunica l'OK
    close(pipe_fd[1]);          // Chiudi il lato di scrittura della pipe
    exit(0);
}
...
void preparaStream(FILE *fin, wchar_t *out)
{
    ...
    while ((c = fgetwc(fin)) != EOF)
    {
        ...
        {
            // se il carattere precedente non è segno di interpunzione
            // probabilmente sto trattando un nome di persona e mantengo il maiuscolo.
            if (!isPunteggiatura(cPrec))
                c = towlower(c);
...
/// @brief funzione per l'inserimento di un carattere nell'array fileNormalizzato. la funzione aumenta le dimensioni dell'array se necessario
/// @param arr  l'array in cui inserire il carattere
/// @param c    il carattere da inserire nell'array
/// @param n_CharFileNormalizzato le dimensioni dell'array
...
/// @brief funzione per il popolamento dell'array di Record Occorrenze.
/// @param testo il testo in input che viene letto
...
/// @brief funzione per la creazione dell'array contenente le parole e la probabilità calcolata per ciascuna parola successiva.
/// la funzione legge il file csv e crea un array di prbabilità per ciascuna parola
void creaArrayProb()
{
    ...
    /*per test: riabilitare la stampa se vuoi visualizzare le percentuali
    printf("\n\nStampa TUTTO\n");
    // stampo tutto
...
/// @brief funzione per la generazione e scrittura su file di output del tsto
void generaTesto()
{
    setlocale(LC_ALL, "en_US.UTF-8");
    ...
    const char *nomeFileOutput = OUTPUT_FILE;
    FILE *file = fopen(nomeFileOutput, "wb");

    printf("\nSto generando il testo:\n");
    ...
    for (int x = 0; x < parametri.nParoleDaGenerare; x++)
    {
        ...
        if (indexParolaIniziale == -1)
            indexParolaIniziale = 0;
        ...
        case 1:
            ...
            fwprintf(file, L" %ls", (wchar_t *)upperParolaEstratta);
            /* code */
            break;

        default:
            if (is_special_char(parolaEstratta) == 1)
                fwprintf(file, L"%ls", (wchar_t *)parolaEstratta);
            else
                fwprintf(file, L" %ls", (wchar_t *)parolaEstratta);
            break;
        }
